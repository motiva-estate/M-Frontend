...
export const Route = createFileRoute("/land/$slug")({
  loader: async ({ params, context }) => {
    const parcel = await context.queryClient.ensureQueryData(landBySlugQueryOptions(params.slug));
  ...
  errorComponent: ({ error }) => (
    <div className="min-h-screen flex items-center justify-center bg-ivory text-ink px-6">
      <div className="text-center max-w-md">
        <h1 className="font-display text-4xl mb-4">This parcel didn't load.</h1>
...
function LandDetail() {
  ...
  const sizes = parcel.sizes ?? [];
  const [size, setSize] = useState<number>(sizes[0] ?? 0);
  const cover = landCover(parcel.slug, resolveImage(parcel.cover, { width: 2000 }) ?? parcel.coverUrl);
  const galleryResolved = (parcel.gallery ?? [])
    .map((g) => resolveImage(g, { width: 1600 }))
    .filter((u): u is string => Boolean(u));
  const gallery = landGallery(parcel.slug, galleryResolved);

  return (
    <>
      <section className="relative h-[72svh] min-h-[520px] bg-ink text-ivory overflow-hidden">
        <img src={cover} alt={parcel.name} className="absolute inset-0 h-full w-full object-cover ken-burns" />
        ...
        <div className="relative z-10 h-full mx-auto max-w-[1500px] px-6 md:px-10 flex flex-col">
          ...
          <div className="mt-auto pb-16 md:pb-20">
            ...
            {parcel.estate && (
              <p className="mt-6 max-w-xl text-[16px] text-ivory/80">Estate land at {parcel.estate}.</p>
            )}
          </div>
        </div>
      </section>
            ...
            <div className="md:sticky md:top-28 space-y-6">
              <div className="bg-ink text-ivory p-8">
                ...
                <p className="text-ivory/70 text-[14px] mb-6">
                  {size ? (
                    <>Selected: <span className="text-ivory">{size} SQM</span>. </>
                  ) : null}
                  Pricing moves with approvals — we'll share current terms directly.
                </p>
                <WhatsAppCta
                  text={landWhatsAppText(parcel.name, size || undefined)}
                  label={size ? `Enquire — ${size} SQM` : "Enquire on WhatsApp"}
                  variant="primary"
                />
                <Link
                  to="/contact"
